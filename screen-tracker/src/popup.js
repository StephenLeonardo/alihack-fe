document.addEventListener("DOMContentLoaded", (e) => {
  chrome.runtime.sendMessage({ action: 'getData' }, (response) => {
    if (response && response.data) {
        ...
        `;
        if (response.data.minutes_saved > 0) {
          popupContentHtml += `<div class="w-full max-w-md mt-6 p-6 bg-gray-100 rounded-lg shadow-lg">
            <h2 class="text-lg font-semibold text-gray-800 mb-4">Summary:</h2>
            <p class="text-gray-700 text-base">${response.data.summary}</p>
          </div>`
        }
        popupContentHtml += `</div>`
        document.getElementById('data-display').innerHTML = popupContentHtml
    }
  });
